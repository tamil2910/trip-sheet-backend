...
public class TripBillingService {
  ...
  /**
   * Creates a payable for each direct delegation only.  A->B never receives
   * information about C/D: every record contains only its payer and payee.
  ...
  public List<PurchaseInvoice> generatePurchaseInvoicesForTrip(UUID tripId) {
    ...
    Trip trip = tripRepository.findById(tripId)
        .orElseThrow(() -> new RuntimeException("Trip not found for purchase invoice billing"));
    TripSummary tripSummary = tripSummaryRepository.findByTripId_Id(tripId)
        .orElseThrow(() -> new RuntimeException("Trip summary not found for purchase invoice billing"));

    List<VendorDelegationHistory> delegations = vendorDelegationHistoryRepository
        .findByTrip_IdAndIsDeletedFalseOrderByDelegatedAtAscCreatedAtAsc(tripId);
    if (delegations.isEmpty()) {
      return List.of();
    }

    // The originating vendor's revenue is the PO value.  Downstream vendors'
    // revenue is the payable generated by the immediately previous valid hop.
    ...
    for (VendorDelegationHistory delegation : delegations) {
      ...
      try {
        ...
        log.info("Skipping purchase invoice for delegation {} -> {} on trip {}: {}",
            delegation.getFromVendor().getId(), delegation.getToVendor().getId(), tripId, ex.getMessage());
      }
    }
    return generated;
  }
  ...
  private PricingContext resolvePartnerPricingContext(Trip trip) {
    return resolvePartnerPricingContext(trip, trip.getAssignedByVendor(), trip.getVendor());
  }

  private PricingContext resolvePartnerPricingContext(Trip trip, Tenant payerVendor, Tenant payeeVendor) {
    if (payerVendor == null || payerVendor.getId() == null || payeeVendor == null || payeeVendor.getId() == null) {
      throw new RuntimeException("Both vendors are required for partner billing");
    }
    VendorPartner vendorPartner = vendorPartnerRepository
        .findByPrimaryVendorAndPartnerVendor(payerVendor, payeeVendor)
